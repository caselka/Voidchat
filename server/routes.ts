...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/create-username-payment', async (req, res) => {
    try {
      ...
      const available = await storage.isHandleAvailable(username);
      if (!available) {
        return res.status(400).json({ message: 'Username is not available' });
      }

      // Create payment intent for username registration
      const paymentIntent = await stripe.paymentIntents.create({
        amount: amount, // $3.00 in cents
        currency: 'usd',
        metadata: {
  ...
  app.post('/api/complete-username-registration', async (req, res) => {
    try {
      ...
      const { username, email } = paymentIntent.metadata;
      ...
      // Create user account
      const userId = `stripe_${paymentIntent.id}`;
      ...
      console.error('Error completing username registration:', error);
